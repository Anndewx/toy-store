  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (error.response) {
        ...
        setError('Connection failed. Please check if the Node.js server is running and accessible.');
      } else {
        // ข้อผิดพลาดอื่นๆ
  ...
  return (
    <div className="min-vh-100 position-relative overflow-hidden">
      ...
      <div className="position-absolute w-100 h-100" style={{ background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', zIndex: -2 }}>
        <div className="position-absolute rounded-circle" style={{ width: '300px', height: '300px', background: 'rgba(255,255,255,0.1)', top: '-150px', right: '-150px', animation: 'float 6s ease-in-out infinite' }}></div>
        <div className="position-absolute rounded-circle" style={{ width: '200px', height: '200px', background: 'rgba(255,255,255,0.05)', bottom: '-100px', left: '-100px', animation: 'float 8s ease-in-out infinite reverse' }}></div>
        <div className="position-absolute rounded-circle" style={{ width: '150px', height: '150px', background: 'rgba(255,255,255,0.08)', top: '20%', left: '10%', animation: 'float 7s ease-in-out infinite' }}></div>
      </div>
                ...
                <div className="card-body p-4">
                  <form onSubmit={handleSubmit}>
                    ...
                    <div className="mb-3">
                      ...
                      <input type="text" className="form-control form-control-lg border-0 shadow-sm" id="username" name="username" value={formData.username} onChange={handleChange} placeholder="Enter your username" disabled={isLoading} style={{ borderRadius: '12px', background: 'rgba(248,249,250,0.8)', padding: '12px 16px' }} />
                    </div>
                    <div className="mb-4">
                      <label htmlFor="password" className="form-label fw-semibold text-dark"><i className="bi bi-lock me-2"></i>Password</label>
                      <input type="password" className="form-control form-control-lg border-0 shadow-sm" id="password" name="password" value={formData.password} onChange={handleChange} placeholder="Enter your password" disabled={isLoading} style={{ borderRadius: '12px', background: 'rgba(248,249,250,0.8)', padding: '12px 16px' }} />
                    </div>
                    <button type="submit" className="btn btn-lg w-100 fw-semibold border-0 shadow-sm mb-3" disabled={isLoading} style={{ borderRadius: '12px', background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', padding: '12px' }}>
                      {isLoading ? (<><span className="spinner-border spinner-border-sm me-2" role="status" aria-hidden="true"></span>Signing in...</>) : (<><i className="bi bi-box-arrow-in-right me-2"></i>Sign In</>)}
                    </button>
                    <div className="text-center">
                      <p className="text-muted mb-0">Don't have an account? <Link to="/register" className="fw-semibold" style={{ color: '#667eea', textDecoration: 'none' }}>Create one here</Link></p>
                    </div>
                  </form>
                </div>
              </div>
